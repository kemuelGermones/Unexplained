...
module.exports.renderReports = async (req, res, next) => {
    ...
    if (category) {
        ...
        const report = await (await Report.find({ category }).populate('author')).reverse();
...
module.exports.editReport = async (req, res, next) => {
    ...
    if (req.body.deleteImages) {
        for (let deleteFilename of req.body.deleteImages) {
            ...
            report.images = report.images.filter(img => img.filename !== deleteFilename);
        }
    }
    ...
    if (mergedImgs.length > 5) {
        ...
        req.flash('error', 'Total Images should be less than or equal to five only');
        return res.redirect(`/reports/${id}`);
    }
    req.body.report.images = mergedImgs;
    const editedReport = await Report.findByIdAndUpdate(id, { ...req.body.report });
